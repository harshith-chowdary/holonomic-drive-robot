...
def inverse_kinematics(x, y, w, theta):
	...
	left_alpha = 210*(PI/180) + theta
	right_alpha = 330*(PI/180) + theta

	arr = np.array([[math.cos(front_alpha + PI/2), math.cos(left_alpha + PI/2), math.cos(right_alpha + PI/2)], [math.sin(front_alpha + PI/2), math.sin(left_alpha + PI/2), math.sin(right_alpha + PI/2)], [1, 1, 1]])
