...
def inverse_kinematics(x, y, w):

	front_alpha = - hola_theta
	right_alpha = 120*(PI/180) - hola_theta
	left_alpha = 240*(PI/180) - hola_theta
...
def main():
	...
	vel_front.torque = [0, 0, 0]
	vel_right.torque = [0, 0, 0]
	vel_left.torque = [0, 0, 0]
	...
	for i in range(0,len(x_goals)):
		...
		while not rospy.is_shutdown():
			...
			front, right, left = inverse_kinematics(vel_x, vel_y, vel_z)

			vel_front.torque = [0, front, 0]
			vel_right.torque = [0, right, 0]
			vel_left.torque = [0, left, 0]

			front_wheel_pub(vel_front)
			right_wheel_pub(vel_right)
			left_wheel_pub(vel_left)
