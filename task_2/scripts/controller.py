...
def inverse_kinematics(x, y, w, theta):
	...
	arr[0] = round(arr[0]*m,4)
	arr[1] = round(arr[1]*m,4)
	arr[2] = round(arr[2]*m,4)
...
def main():
	...
	for i in range(0,len(x_goals)):
		...
		theta_d = (theta_diff + hola_theta)%PI
		'''
		...
		if abs(x_d - hola_x) < 5 and abs(y_d - hola_y) < 5:
		...
		while not rospy.is_shutdown():
			...
			if (abs(theta_err) < 2*PI/180):
				angular_speed = 0
				f1 = 1
				
			if (abs(x_err_gf) < 2 and abs(y_err_gf) < 2):
			...
			if not f2:
				...
				beta = math.atan2(y_err_gf, x_err_gf)
				vel_x = speed*math.cos(beta)
				vel_y = speed*math.sin(beta)
				...
				if (dist>10 and theta_err>10*PI/180):
